...
function Billboard({ getMovies, movies }){
    ...
    const observer = new IntersectionObserver((entries) => {
        ...
        if(entry[0]){
            ...
            entries[0].target.className = 'billboard--container billboard--container__notrender';
        }
    });
    ...
    return(
        <div className="billboard">
            ...
            {movies.length > 0 ? movies.map((m) =>
                ...
                <div key={m.movie_id} className='billboard--container' ref={handleMovieRef}>
                    <img className='billboard--poster' src={m.poster} alt={m.title}/>
                    <div className='billboard--info'>
                        <h3 className='billboard--title'>{m.title.length > 22 ? m.title.toUpperCase().slice(0, 22).concat('...') : m.title}</h3>
                        <p className='billboard--description'>{m.description.slice(0, 140).concat('...')}</p>
